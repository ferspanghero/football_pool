...
- [ ] **BL6**: First-team-to-score bonus. Let a player predict which side scores first in a match (home / away / no goal); award bonus points if correct. **Touches:** a new field on the prediction (alongside the score), a new field on `match_results` for the actual first scorer (**not** derivable from the 90-minute score, so the admin must record it) + the Admin Results UI, and the scoring engine (`shared/scoring.ts`). **Open questions:** point value and whether it's phase-weighted; 0-0 / abandoned handling; locks at the match's kickoff like the score prediction.
- [ ] **BL7**: Per-phase 2× boost. In each phase (`GROUP_R1`, `GROUP_R2`, …, `FINAL`) a player may flag one match to double the points it earns. **Touches:** store the boosted `matchId` per (player, phase) — new table/column with a one-per-phase constraint; `computeLeaderboard` applies the 2× for that match; My-picks UI gets a per-row boost toggle that enforces a single pick per phase. **Open questions:** when the choice locks (per-match at its kickoff, or the whole phase at its first kickoff); whether 2× stacks with or replaces the phase multiplier; interaction with the champion bonus.
...
- [ ] **BL9**: First-class mobile experience. Desktop-first was a deliberate v1 choice (`app.css`: 960px container, a single 640px breakpoint) — the app is *usable* on phones but not polished. **Goal:** make the core flows comfortable on small screens. **Likely touches:** the My-picks `picks-grid` (many columns per row — cramped when narrow), the Admin Results/Players rows, the Groups (3×4) and Knockouts layouts, the leaderboard table, and tap-target sizing on the score inputs/buttons (numeric `inputmode` is already set). **Open questions:** responsive-CSS reflow vs. purpose-built narrow layouts for the densest views; add mobile-viewport E2E (Playwright device emulation) to lock it in.
