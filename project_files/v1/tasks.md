...
- [ ] **X2**: Run `./scripts/deploy.sh` — creates the D1 database, applies migrations, sets secrets (`SESSION_SECRET`, `ADMIN_PASSWORD_HASH`), and deploys the single Worker (SPA static assets + API). *Requires one interactive `wrangler login`; documented in README.*
...
- [ ] **BL6**: Rate limiting on the auth endpoints (defense-in-depth brute-force dampening). **Approach:** the Cloudflare Workers Rate Limiting binding declared in `wrangler.toml` (`[[ratelimits]]`) — infra-as-code, free tier, works without a custom domain — *not* the dashboard WAF rules (zone-only). **Tiered, not blanket:** a tight IP-keyed limit on the secret-guessing routes (`POST /api/admin/login`, `POST /api/games/:id/enter`); optionally a looser identity-keyed limit on authenticated writes; leave public reads to the platform. Surface rejections with the already-reserved `RATE_LIMITED` (429) error code. **Caveats:** `period` is restricted to 10 or 60s; limits are per-Cloudflare-location and eventually consistent; friends behind one NAT'd IP share a bucket, so keep IP limits generous.
